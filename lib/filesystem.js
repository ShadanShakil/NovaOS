...
// Remove a file or folder from the files array (including nested children if target is a folder)
export function removeNode(files, currentPath, name) {
  ...
  if (targetNode.type === 'folder') {
    const folderFullPath = currentPath === '/' ? `/${targetNode.name}` : `${currentPath}/${targetNode.name}`;
    newFiles = files.filter((f) => {
      // Remove the folder itself
      if (f.path === currentPath && f.name.toLowerCase() === normalizedName.toLowerCase()) {
        return false;
      }
      // Remove any file/folder inside it
      if (f.path === folderFullPath || f.path.startsWith(folderFullPath + '/')) {
        return false;
      }
      return true;
    });
